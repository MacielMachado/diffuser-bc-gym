...
class CarRacingCustomDataset(Dataset):
    def __init__(
        ...
        if train_or_test == "train":
            self.image_all = self.image_all[:n_train]
            self.action_all = np.expand_dims(self.action_all[:n_train], axis=-1)[:n_train]
        elif train_or_test == "test":
            self.image_all = self.image_all[n_train:]
            self.action_all = self.action_all[n_train:]
        else:
        ...
        # all_images = []
        # self.obs = DataHandler().to_greyscale(self.obs)
        # self.obs = DataHandler().normalizing(self.obs)
        # self.obs = DataHandler().stack_with_previous(self.obs)
        # self.obs = self.obs[90:]
        # self.action = self.action[90:]

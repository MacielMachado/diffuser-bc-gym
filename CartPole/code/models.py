...
class Model_mlp_diff_embed(nn.Module):
    ...
    def forward(self, y, x, t, context_mask):
        ...
        context_mask = context_mask.repeat(1, 1).T
...
class Model_no_cnn(nn.Module):
    def __init__(self, x_shape, n_hidden, y_dim, embed_dim, net_type):
        super(Model_no_cnn, self).__init__()
        ...
        self.embed = nn.Sequential(
            nn.Linear(x_shape, self.embed_dim),
            nn.LeakyReLU(),
            nn.Linear(self.embed_dim, self.embed_dim),
        )

        cnn_out_dim = self.embed_dim
